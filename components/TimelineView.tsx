            ...
            <div className="flex brutalist-border bg-white overflow-hidden h-full">
              {(['all', 'draft', 'published'] as const).map((status) => (
                <button
                  ...
                  className={`px-6 py-4 text-[11px] font-black uppercase border-r last:border-r-0 border-black transition-all ${selectedStatus === status ? 'bg-amber-300' : 'hover:bg-zinc-50'
                    }`}
                >
                  {status}
                </button>
        ...
        {allTags.length > 0 && (
          <div className="flex flex-wrap gap-2 pt-5 border-t-4 border-black border-dotted">
            <button
              ...
              className={`px-4 py-1.5 text-[10px] font-black uppercase transition-all border-2 border-black ${selectedTag === null ? 'bg-black text-white' : 'bg-white hover:bg-zinc-100'
                }`}
            >
              SHOW_ALL
            </button>
            {allTags.map(tag => (
              <button
                ...
                className={`px-4 py-1.5 text-[10px] font-black uppercase transition-all border-2 border-black ${selectedTag === tag ? 'bg-amber-300' : 'bg-white hover:bg-zinc-100'
                  }`}
              >
                #{tag}
              </button>
            ...
            >
              <div className="p-8 md:p-10 flex-1 flex flex-col">
                <div className="flex justify-between items-center mb-8">
                  <span className={`px-4 py-1.5 brutalist-border text-[10px] font-black uppercase italic ${study.status === 'published' ? 'brutalist-bg-success text-black' : 'brutalist-bg-accent text-black'
                    }`}>
                    {study.status}
                  </span>
